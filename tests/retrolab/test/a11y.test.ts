...
test.describe.parallel("accessibility checks", () => {
   ...
   */
  test("Check Lorenz notebook accessibility before running all cells", async ({ page, tmpPath }, testInfo) => {
   ...
   * @see https://github.com/abhinaba-ghosh/axe-playwright
   */
  // test("Check Lorenz notebook accessibility after running all cells", async ({ page, tmpPath }, testInfo) => {
  //   await page.goto(`notebooks/${tmpPath}/${LORENZ}`);
